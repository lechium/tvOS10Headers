/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, March 22, 2017 at 9:05:55 AM Mountain Standard Time
* Operating System: Version 10.1 (Build 14U593)
* Image Source: /System/Library/Frameworks/TVServices.framework/TVServices
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <TVServices/TVSSecureKeyLoaderDelegate.h>

@protocol TVSSecureKeyDeliveryCoordinatorDelegate;
@class NSObject, TVSPlaybackReportingEventCollection, TVSSecureKeyLoader, NSData, NSMutableSet, NSString;

@interface TVSSecureKeyDeliveryCoordinator : NSObject <TVSSecureKeyLoaderDelegate> {

	BOOL _certFetchInProgress;
	NSObject*<TVSSecureKeyDeliveryCoordinatorDelegate> _delegate;
	TVSPlaybackReportingEventCollection* _eventCollection;
	TVSSecureKeyLoader* _secureKeyLoader;
	NSData* _certificateData;
	NSMutableSet* _requestsAwaitingCertFetch;

}

@property (nonatomic,retain) TVSSecureKeyLoader * secureKeyLoader;                                            //@synthesize secureKeyLoader=_secureKeyLoader - In the implementation block
@property (nonatomic,retain) NSData * certificateData;                                                        //@synthesize certificateData=_certificateData - In the implementation block
@property (assign,nonatomic) BOOL certFetchInProgress;                                                        //@synthesize certFetchInProgress=_certFetchInProgress - In the implementation block
@property (nonatomic,retain) NSMutableSet * requestsAwaitingCertFetch;                                        //@synthesize requestsAwaitingCertFetch=_requestsAwaitingCertFetch - In the implementation block
@property (assign,nonatomic,__weak) NSObject*<TVSSecureKeyDeliveryCoordinatorDelegate> delegate;              //@synthesize delegate=_delegate - In the implementation block
@property (nonatomic,retain) TVSPlaybackReportingEventCollection * eventCollection;                           //@synthesize eventCollection=_eventCollection - In the implementation block
@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
+(void)initialize;
+(BOOL)isSecureKeyDeliveryRequest:(id)arg1 ;
-(void)setDelegate:(NSObject*<TVSSecureKeyDeliveryCoordinatorDelegate>)arg1 ;
-(id)init;
-(NSObject*<TVSSecureKeyDeliveryCoordinatorDelegate>)delegate;
-(void)secureKeyLoader:(id)arg1 didLoadKeyResponseData:(id)arg2 renewalDate:(id)arg3 forRequest:(id)arg4 ;
-(void)secureKeyLoader:(id)arg1 didFailWithError:(id)arg2 forRequest:(id)arg3 ;
-(void)secureKeyLoader:(id)arg1 didLoadCertificateData:(id)arg2 forRequest:(id)arg3 ;
-(TVSPlaybackReportingEventCollection *)eventCollection;
-(void)secureKeyLoader:(id)arg1 didLoadContentIdentifierData:(id)arg2 forRequest:(id)arg3 ;
-(void)sendStopRequest;
-(void)setEventCollection:(TVSPlaybackReportingEventCollection *)arg1 ;
-(NSData *)certificateData;
-(void)setCertificateData:(NSData *)arg1 ;
-(id)initWithSecureKeyLoader:(id)arg1 ;
-(void)loadSecureKeyRequest:(id)arg1 ;
-(void)_loadSecureKeyRequest:(id)arg1 sendStartReportingEvent:(BOOL)arg2 ;
-(TVSSecureKeyLoader *)secureKeyLoader;
-(void)setCertFetchInProgress:(BOOL)arg1 ;
-(NSMutableSet *)requestsAwaitingCertFetch;
-(BOOL)certFetchInProgress;
-(void)_finishLoadingWithError:(id)arg1 forRequest:(id)arg2 ;
-(void)setSecureKeyLoader:(TVSSecureKeyLoader *)arg1 ;
-(void)setRequestsAwaitingCertFetch:(NSMutableSet *)arg1 ;
@end

